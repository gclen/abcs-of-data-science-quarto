...
# R is for Reproducibility {#sec-r-is-for-reproducibility}
...
* It’s not enough to have the same raw data, the processing pipeline should be the same (see @sec-m-is-for-munging-data). For example, how did the original authors handle missing data?
...
* Stochastic gradient descent (see @sec-g-is-for-gradient-descent) is commonly used for optimization (e.g. in most deep learning models)
* In many embedding and dimension reduction (see @sec-e-is-for-embeddings) algorithms there is randomness in the output. Points that are close in the higher dimensional space will still be close to points in the lower dimensional space, but the position of the points themselves may change
