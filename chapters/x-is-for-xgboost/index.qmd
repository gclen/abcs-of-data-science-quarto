...
# X is for XGBoost {#sec-x-is-for-xgboost}
...
If we want to train an ensemble of decision trees how should we do it? We can generalize boosting on decision trees to use arbitrary loss functions (@sec-s-is-for-supervised-learning). Once we’ve picked a loss function we can use gradient descent (@sec-g-is-for-gradient-descent) to optimize it. Using this technique is typically called **Gradient Boosted Decision Trees** or **Gradient Boosted Trees**. 
