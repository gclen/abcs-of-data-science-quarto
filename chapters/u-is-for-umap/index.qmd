...
* Uniform Manifold Approximation and Projection (UMAP). Developed by [Leland McInnes](https://github.com/lmcinnes) and [John Healy](https://github.com/jc-healy) (who also made HDBSCAN (@sec-h-is-for-hdbscan) it has been growing in popularity in recent years. It’s faster than t-SNE and arguably preserves the higher dimensional structure better than t-SNE.
...
We want to find groups of related things, which sounds like we’ll need to use a clustering algorithm (@sec-c-is-for-clustering). The clustering algorithm can’t just use the raw data on its own, so we’ll need to pass in an embedding. You might recall from @sec-e-is-for-embeddings that an embedding requires two things:
